...
function [ distance ] = Metric( A, B ) % calculate distance between 2 points
    distance = sqrt((A(1)-B(1))^2 + (A(2)-B(2))^2); % euclidean distance
end

function [ T ] = DecreaseTemperature( initialTemperature, k)
    T = initialTemperature * 0.01 / k; 
end
...
function [ seq ] = GenerateStateCandidate(seq)
    ...
    % swap 2 points
    if(i < j) 
        seq(i:j) = flipud(seq(i:j)); % reverse
    else
        seq(j:i) = flipud(seq(j:i)); % reverse
    end
end
